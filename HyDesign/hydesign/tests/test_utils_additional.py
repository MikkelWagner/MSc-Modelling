...
from hydesign.ems.utils import expand_to_lifetime
...
def test_seasonal_weeks_follow_meteorological_months():
    representative_weeks = np.repeat([0.0, 1.0, 2.0, 3.0], 7 * 24)
    year = expand_to_lifetime(
        representative_weeks,
        life=365 * 24,
        weeks_per_season_per_year=1,
    )

    boundaries = np.cumsum([0, 59, 92, 92, 91, 31]) * 24
    expected_seasons = [0.0, 1.0, 2.0, 3.0, 0.0]
    assert len(year) == 365 * 24
    for start, end, season in zip(boundaries[:-1], boundaries[1:], expected_seasons):
        assert np.all(year[start:end] == season)
...
def test_generate_availability_ensamble_small():
    ...
    assert ds.sizes["seed"] == 2
    assert ds.sizes["component"] == 2
    assert ds.TTF_indices.shape == ds.TTR_indices.shape


def test_generate_availability_ensamble_in_batches():
    ds = generate_availability_ensamble(
        ts_start="2030-01-01 00:00",
        ts_end="2030-01-01 00:00",
        seeds=[3],
        MTTF=1,
        MTTR=1,
        N_components=401,
    ...
    assert ds.sizes["batch_no"] == 2
    assert ds.sizes["component"] == 400
    np.testing.assert_array_equal(ds.final_seed, [[1003, 2003]])
