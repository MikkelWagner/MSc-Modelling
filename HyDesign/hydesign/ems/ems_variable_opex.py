...
class ems:
    ...
    def compute(
        ...
        marginal_displacement_factor_t=None,
        emissions_value=0.0,
        **kwargs,
    ):
        ...
        emissions_value = np.asarray(emissions_value).item()
        if marginal_displacement_factor_t is None:
            marginal_displacement_factor_t = np.zeros_like(price_t)
        ...
        WSPr_df["wind_t_ref"] = wind_t_ref
        WSPr_df["M"] = M
        WSPr_df["solar_t"] = solar_t
        WSPr_df["price_t"] = price_t
        WSPr_df["marginal_displacement_factor_t"] = marginal_displacement_factor_t
        ...
        ems_kwargs = {}
        if self.ems_type == "cplex":
            ems_kwargs["yaw_mode"] = self.yaw_mode
            ems_kwargs["batch_size"] = self.batch_size
        ...
        (
            ...
            wind_ts_ref=WSPr_df.wind_t_ref,
            M=WSPr_df.M,
            ...
            marginal_displacement_factor_ts=WSPr_df.marginal_displacement_factor_t,
        ...
        outputs["wind_t_ext"] = expand_to_lifetime(
            wind_t * lambda_ts + wind_t_ref * (1 - lambda_ts),
            life_y=self.life_y,
        ...
        super().__init__(
            inputs=[
                ...
                (
                    "M",
                    {
                        "desc": "ONM cost of control time series",
                        "shape": [model.N_time],
                    },
                ),
                (
                ...
                (
                    "marginal_displacement_factor_t",
                    {
                        "val": np.zeros(model.N_time),
                        "desc": "Marginal emissions displaced by export [kgCO2e/MWh]",
                        "shape": [model.N_time],
                    },
                ),
                (
                    ...
                    {
                        "val": 0.0,
                        "desc": "EMS shadow value of displaced emissions [EUR/tCO2e]",
                    },
                ),
            ],
            outputs=[
                ...
                (
                    "lambda_t",
                    {
                        "desc": "binary parameter indicating when optimal control was used (1) or not (0)",
                        "shape": [model.N_time],
                    },
                ),
            ],
...
def ems_cplex(
    ...
    batch_size=110,
    marginal_displacement_factor_ts=None,
    emissions_value=0.0,
    yaw_mode="smart",
):

    if marginal_displacement_factor_ts is None:
        marginal_displacement_factor_ts = np.zeros(len(price_ts))
    if len(marginal_displacement_factor_ts) != len(price_ts):
        raise ValueError(
            "marginal_displacement_factor_ts must have the same length as price_ts"
        )
    marginal_displacement_factor_ts = pd.Series(
        np.asarray(marginal_displacement_factor_ts), index=price_ts.index
    )
    ...
    lambda_ts = np.zeros(len(wind_ts))

    for ib, batch in enumerate(batches):
        wind_ts_sel = wind_ts.iloc[batch]
        wind_ts_ref_sel = wind_ts_ref.iloc[batch]
        M_sel = M.iloc[batch]
        solar_ts_sel = solar_ts.iloc[batch]
        price_ts_sel = price_ts.iloc[batch]
        marginal_displacement_factor_ts_sel = marginal_displacement_factor_ts.iloc[
            batch
        ]
        ...
        (
            ...
            cost_of_battery_P_fluct_in_peak_price_ratio=cost_of_battery_P_fluct_in_peak_price_ratio,
            n_full_power_hours_expected_per_day_at_peak_price=n_full_power_hours_expected_per_day_at_peak_price,
            marginal_displacement_factor_ts=marginal_displacement_factor_ts_sel,
            emissions_value=emissions_value,
            yaw_mode=yaw_mode,
        )
        ...
        lambda_ts[batch] = lambda_ts_batch
...
def ems_cplex_parts(
    ...
    cost_of_battery_P_fluct_in_peak_price_ratio : cost of battery power fluctuations computed as a peak price ratio
    n_full_power_hours_expected_per_day_at_peak_price : Penalty occurs if number of full power hours expected per day at peak price are not reached
    marginal_displacement_factor_ts : marginal emissions displaced by exported energy [kgCO2e/MWh]
    emissions_value : shadow value assigned to displaced emissions [EUR/tCO2e]
    yaw_mode : ``"reference"`` forces zero yaw, ``"controlled"`` forces the
        precomputed optimal yaw, and ``"smart"`` lets the EMS choose hourly
    ...
    if yaw_mode not in {"reference", "controlled", "smart"}:
        raise ValueError("yaw_mode must be 'reference', 'controlled', or 'smart'")
    ...
    # Convert kgCO2e/MWh and EUR/tCO2e to an export value in EUR/MWh.
    emissions_export_value_ts = (
        marginal_displacement_factor_ts
        * float(np.asarray(emissions_value).item())
        / 1000.0
    )
    ...
    lambda_ts = mdl.binary_var_dict(time, name="lambda_ts", lb=0, ub=1)
    if yaw_mode == "reference":
        mdl.add_constraints(lambda_ts[t] == 0 for t in time)
    elif yaw_mode == "controlled":
        mdl.add_constraints(lambda_ts[t] == 1 for t in time)

    mdl.maximize(
        # revenues and OPEX
        mdl.sum(
            (price_ts[t] + emissions_export_value_ts[t]) * P_HPP_t[t]
            - lambda_ts[t] * M[t]
            for t in time
        )
    ...
    for t in time:
        ...
        mdl.add_constraint(
            P_HPP_t[t]
            == wind_ts[t] * lambda_ts[t]
            + wind_ts_ref[t] * (1 - lambda_ts[t])
            + solar_ts[t]
            + -P_curtailment_t[t]
    ...
    lambda_ts_df = solution_series(sol, lambda_ts, time)
    ...
    lambda_ts = lambda_ts_df.reindex(time, fill_value=0).values

    if len(P_HPP_ts_df) < len(wind_ts):
        # print('recomputing p_hpp')
        P_HPP_ts = (
            wind_ts.values * lambda_ts
            + wind_ts_ref.values * (1 - lambda_ts)
            + solar_ts.values
            + -P_curtailment_ts
            + P_charge_discharge_ts
        )
...
def solution_series(solution, variables, index):
    """Return all requested solution values, including solver-omitted zeros."""
